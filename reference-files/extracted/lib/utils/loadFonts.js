...
export async function loadFonts(ctx) {
    let fontFacesCSS = '';
    ...
    if (fontFaces) {
        ...
        fontFacesCSS += `* {
            font-family: '${selectedCustomFont ? selectedCustomFont.family : fontFaces[0].family}', 'helvetica neue', 'helvetica', 'arial', 'sans-serif' !important;
        }`;
...
async function appendFontLinks(fontFaces, head, fontFacesCSS) {
...
function createLinkElement(font) {
...
function getFontInfo(font) {
    const url = font.url || '';
    const extension = font.guid && font.guid !== null && font.format
